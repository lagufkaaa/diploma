...
class NFP:
    ...
    SCALE = 1e6  # Используем константу масштаба для всех преобразований
    ...
    def polygon_to_path(polygon):
        ...
        outer = to_int_coords(polygon.exterior.coords[:-1])  # Убираем замыкающую точку
    ...
    def minkowski_difference(polygon1, polygon2, anchor_point=(0, 0)):
        ...
        # Инвертируем координаты polygon1 для разности Минковского
        ...
        # Обратное масштабирование и фильтрация
        ...
        if not polygons:
            return Polygon()  # Пустой результат
    ...
    def outer_no_fit_polygon(polygon1, polygon2, anchor_point=(0, 0)):
        """
        Вычисляет Outer No-Fit Polygon (ONFP) — область, в которую нельзя поместить polygon1 без пересечения с polygon2.
        """
        if not isinstance(polygon1, Polygon):
